    ...
    public ref struct XmlReadBuffer
    {
        ...
        private int ReadInto(ReadOnlySpan<char> span, IXmlSerializable obj)
        {
            ...
            for (var i = 0; i < span.Length;)
            {
                ...
                var spaceIdx = IndexOfLastSpaceOrTab(currSpan, out var numberOfSpacesOrTab);
                if (spaceIdx > closeBraceIdx)
                {
                    spaceIdx = -1;
                    numberOfSpacesOrTab = 1;
                } // we are looking for a space in the node declaration
                ...
                var nodeName = currSpan.Slice(1, nameEndIdx - numberOfSpacesOrTab);
...
#pragma warning restore 162
        ...
        /// <summary>
        /// Check if the current char is a whitespace or tab
        /// </summary>
        /// <returns>True if the current char is a whitespace or tab</returns>
        ...
        /// <summary>
        /// Return the index of the last whitespace or tab character in the span before next non-whitespace character
        /// </summary>
        /// <param name="span">Text to parse</param>
        /// <param name="numberOfSpacesOrTab">Out the number of sequential spaces or tab founds</param>
        /// <returns>Index of the last whitespace or tab character in the span before next non-whitespace character</returns>
        private static int IndexOfLastSpaceOrTab(ReadOnlySpan<char> span, out int numberOfSpacesOrTab)
        {
            var lastIndex = -1;
            var isFirstSpaceOrTab = true;
            numberOfSpacesOrTab = 0;
            for (int i = 0; i < span.Length; i++)
            {
                if (IsSpaceOrTab(span[i]))
                {
                    numberOfSpacesOrTab++;
                    lastIndex = i;
